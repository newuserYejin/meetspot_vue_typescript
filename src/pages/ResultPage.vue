...
<template>
  <div>
    <div
      class="flex flex-col py-[20px] px-[24px] h-100% max-w-[1200px] mx-auto"
    >
      <div
          ...
          <div class="flex gap-[5px]">
            ...
            <div>
              {{
                todayFriendList
                  .map(
                    (friend) => `${friend.friend_name}(${friend.station_nm})`
                  )
                  .join(", ")
              }}
            </div>
      ...
      >
        ...
        <div class="flex flex-col">
          ...
          <span>
            모든 친구가 가장 빠르게 모일 수 있는 장소입니다.(가장 오래 걸리는
            친구 기준 : {{ finalTime }}분)
          </span>
        </div>
      </div>
        ...
        <div class="flex gap-[16px]">
          ...
          >
            <div class="text-[24px] text-[#6C5CE7] font-bold">
              {{
                getAvg(
                  finalPlaceRoute?.map((route) => route.exChange_cnt) ?? []
                )
              }}
            </div>
